...
class TF_trans():
    ...
    def get_T_cam_base(self):
        ...
        while not rospy.is_shutdown():
            try:
                trans = tfBuffer.lookup_transform("base_link","camera_depth_optical_frame", rospy.Time()) # Check if transform exists
                # print(f"translation : {trans}")
            except Exception as e:
                print(f"the error is : {e}")
                continue
            input_pose = PoseStamped()
            transformed_pose = do_transform_pose(input_pose,trans)   
    ...
    def get_ximg(self,id):
        ...
        """        
        try:
            # data = self.read_json_file(os.path.join(os.getcwd(),"config/marker_poses.json"))        
    ...
    def Ximg2Xbase(self)->np.ndarray:
        ...
        try:
            ...
            # print(f't_cam_bae:{T_cam_base}')
            print(T_cam_base[0][3])
            Z = self.poses[2]
            # print(f'z:{Z}')
            Ximg = np.array([self.poses[0], self.poses[1], 1.0])
            # print(Ximg)
            # Xcam = (self.inst_matrix@ Ximg)
            # print(f'Xcamm:{Xcamm}')
            Xcam = Z * np.linalg.inv(self.inst_matrix) @ Ximg
            # print(f'Xcam:{Xcam}')
            Xbase = T_cam_base @ np.array([Xcam[0], Xcam[1], Xcam[2], 1.0]) 
            print(f'Xbase:{Xbase}')
            Xbase = T_cam_base @ np.array([self.poses[0], self.poses[1], self.poses[2], 1.0])          
            return Xbase[:3]
        except Exception as e:
